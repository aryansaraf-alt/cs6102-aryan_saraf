# imports
import sys

inv_sbox = [
    ,
]

########################
# write your code here #
########################

# pairwise hamming weight sei


def popcount(x):
    return bin(x).count("1")


################################
# do no edit anything below this


def hex_to_byte(hex_str, pos=0):
    """Extract a specific byte from hex string"""
    byte_str = hex_str[2 * pos : 2 * pos + 2]
    return int(byte_str, 16)


# read ciphertexts (only need the 0th byte from each)
filename = sys.argv[1]
with open(filename, "r") as f:
    lines = f.read().splitlines()
    # the last line is not read as it contains the correct key byte which is to be used for verification.
    ciphertext_bytes = [hex_to_byte(line, 0) for line in lines[:20]]

key_guess = 0

#################################

# you may write your code below this to complete your assignment


def pairwise_score(keyguess):
    intermediate = [inv_sbox[ct ^ keyguess] for ct in ciphertext_bytes]
    score = 0
    for i in range(len(intermediate)):
        for j in range(i + 1, len(intermediate)):
            if popcount(intermediate[i] ^ intermediate[j]) == 2:
                score += 1
    return score


best_score = -1
for keyguess in range(256):
    score = pairwise_score(keyguess)
    if score > best_score:
        best_score = score
        key_guess = keyguess

#########################
# do not edit this line #
#########################
print(f"0x{key_guess:02x}")
